...
Tree treeCreationMenu(int n){
    ...
    if(input==2){
        ...
        for (i=0; i<n; i++) {
            ...
            while(!getInt(&input)){ printf("Il valore digitato non Ë un intero, riprovare.\n");};
...
void graphic(Tree tree, char *str, int last){
    char tmp[100];
    if(tree!=NULL){
        //Cambia la modalit‡ di stampa se Ë l'ultimo figlio
        if(!last)
            printf("%s--%d\n", str, tree->info);
        else
            printf("%s\\-%d\n", str, tree->info);

        //Stampa i sottoalberi solo se almeno uno dei due non Ë vuoto
